...
uniform sampler2D u_depth_texture;// 深度圖
...
void main(){
  ...
  // D. 讀取深度圖 (Hidden Information)
  float depth=texture2D(u_depth_texture,texUV).r;
  ...
  float hybridValue=mix(n,lum,CONTENT_WEIGHT);
  
  // --- 加入隱藏資訊 ---
  float finalValue=mix(hybridValue,depth,HIDDEN_WEIGHT);
