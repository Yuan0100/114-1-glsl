...
uniform sampler2D u_tex0;// ../../data/reference/IMG_0712.JPG
...
void main(){
  ...
  vec2 cellSize=1./u_resolution.xy*48.;
  vec2 cell=floor(uv/cellSize)*cellSize;
  
  // Parameters (tweak as needed)
  float ease=.0001;
  float minDist=.005;// normalized color distance threshold
  ...
  float rAve=0.,gAve=0.,bAve=0.;
  float rVelAve=0.,gVelAve=0.,bVelAve=0.;
  float rSep=0.,gSep=0.,bSep=0.;
  int count=0;
  ...
  nextRVel=clamp(nextRVel,-1.,1.);
  nextGVel=clamp(nextGVel,-1.,1.);
  nextBVel=clamp(nextBVel,-1.,1.);
  
  // FBO logic
  #if defined(BUFFER_0)
  if(u_time<1.)gl_FragColor=texture2D(u_tex0,st);
  else gl_FragColor=vec4(nextR,nextG,nextB,1.);
  
  #elif defined(BUFFER_1)
  if(u_time<1.)gl_FragColor=vec4(0.,0.,0.,1.);
  else gl_FragColor=vec4(nextRVel,nextGVel,nextBVel,1.);
